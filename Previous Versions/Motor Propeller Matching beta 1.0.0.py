...
- Pull directly from UIUC-style eta, Ct, Cp data
  ...
  * After essential functions finished, find out how to pull photos
...
# def T(n, R, Ct, rho=1.225):  # May not be useful in this form
#     """
#     :param n: rotations per second
...
propeller = read_propeller_data("custom.txt")

motor = {
    'name': "KDE 2550",
    'kV': 2550 / 60,  # convert to rps / volt
    'i0': 1.2,  # A
    'R': 0.044,  # Ohms
    'max RPM': 55590  # RPM
}

target_speed = 22.35 # m/s
T_req = 2.3  # N
# END FUDGY INPUT
...
#     'max RPM': float(input("Enter the maximum RPM of the motor (kV * max voltage if not given): "))  # RPM
# }
#
#
...
p1.set(title="Thrust for {} at {:.2f} m/s".format(propeller['name'], target_speed),
       ...
       ylabel='Thrust (N)',
       xlim=(0, max_n * 60 * 1.1),
       ylim=(0, max(T(n, handle_array=True))))

p1.plot(n * 60, T(n, handle_array=True),
        label='prop thrust at target speed')
p1.plot(n * 60, T_req * np.ones(n.shape), linestyle='dashed',  # / 4 is there
...
p2.plot((required_RPM, required_RPM), p3.get_ylim(), color='red', label='Required RPM')
